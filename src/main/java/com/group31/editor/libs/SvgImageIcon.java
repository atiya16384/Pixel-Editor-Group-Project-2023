...
public class SvgImageIcon extends ImageIcon {
  ...
  private void update() {
    ...
    try {
      ...
      LOG.log(
        Level.WARNING,
        String.format("Could not get SVG image [%s] due to [%s]", name, ex.getMessage())
      );
    }
  }
  ...
  public synchronized void paintIcon(Component c, Graphics g, int x, int y) {
    ...
    if (
      clipBounds != null &&
      !clipBounds.intersects(new Rectangle(x, y, getIconWidth(), getIconHeight()))
    ) {
      return;
    }
  ...
  /**
   * Creates an icon from a SVG resource.
   * @param resource Path to the resource with NO prefix (e.g. "images/arrow.svg")
   * @param dimensions The dimensions of the icon to generate
   * @author jennin16
   * @return
   */
  public static ImageIcon createIconFromSvg(
    String resource,
    java.awt.Dimension dimensions
  ) {
    try {
      return new SvgImageIcon(
        resource,
